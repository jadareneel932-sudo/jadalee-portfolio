...
const experience = [
  {
    ...
    desc: "Developed and executed campaigns for National Broadway tours, dances, and concerts. Created press releases, promotional infographics, and event materials for the Home Decor & Celebration showcase — reaching audiences across Central Pennsylvania.",
  },
  {
    icon: "🎥",
                      ...
                      <div className="mt-5 grid grid-cols-3 gap-2">
                        {(e as any).images.map((src: string, idx: number) => (
                          <div key={idx} className="overflow-hidden rounded-lg aspect-square">
                            <img src={src} alt={`Sports Information ${idx + 1}`} className="w-full h-full object-cover hover:scale-105 transition-transform duration-500" loading="lazy" />
                          </div>
                        ))}
                      </div>
                    )}
